		...
		for i in range(r):
			for j in range(c):
				if self.grid[i][j].isTerminal:
					...
					elif self.grid[i][j].util = -1: uqdateU[i][j] = -1
		d = 0

		while d >= epsilon*(1-self.gamma)/self.gamma :
			for i in range(r):
				for j in range(c):
			...
			for i in range(r):
				for j in range(c):
					northU, southU, westU, eastU = -1 , -1, -1, -1
					if i > 0 and not self.grid[i-1][j].isWall:
						northU = self.mprobs[0]*self.grid[i-1][j].util
						eastU = self.mprobs[1]*self.grid[i-1][j].util
						southU = self.mprobs[2]*sself.grid[i-1][j].util
						west = self.mprobs[2]*sself.grid[i-1][j].util

					if j > 0 and not self.grid[i][j-1].isWall:
						westU = self.mprobs[0]*self.grid[i][j-1].util
						northU = self.mprobs[1]*self.grid[i][j-1].util
						eastU = self.mprobs[2]*sself.grid[i][j-1].util
						southU = self.mprobs[2]*sself.grid[i][j-1].util

					if i < r-2 and not self.grid[i+1][j].isWall:
						southU = self.mprobs[0]*self.grid[i+1][j].util
						westU = self.mprobs[1]*self.grid[i+1][j].util
						northU = self.mprobs[2]*sself.grid[i+1][j].util
						eastU = self.mprobs[2]*sself.grid[i+1][j].util

					if j < c-2 and not self.grid[i][j+1].isWall:
						eastU = self.mprobs[0]*self.grid[i][j+1].util
						southU = self.mprobs[1]*self.grid[i][j+1].util
						westU = self.mprobs[2]*sself.grid[i][j+1].util
						northU = self.mprobs[2]*sself.grid[i][j+1].util
